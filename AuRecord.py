    ...
    def __init__(self):
        ...
        if not path.exists(r'C:\Ok周志\SysOk\OldUser.pkl'):
            cs.print(Panel.fit('[bold]'
                               ...
                               '\n11. 若当您在使用过程中遇到BUG请联系我们，真诚感谢您的反馈\n12. 任何事物都有两面性，写周志也是一个好习惯，希望您只是时间紧迫才使用我\n'
        ...
        if self.inprr != '1':
            ...
            if not path.exists(r'C:\Ok周志\SysOk\OldUser.pkl'):
                inpww = cs.input(
                    '\n[bold blue]【Ok周志】: 是否愿意关注开发人员微信公众号和b站: Okroie\n1: 我愿意，2: 不愿意\n请选择: ').strip()
                if inpww == '1':
    ...
    def write(self):
        ...
        if not path.exists(r'C:\Ok周志\SysOk\OldUser.pkl'):
            with open(r'C:\Ok周志\SysOk\OldUser.pkl', 'w'):
                pass
        sleep(2)
        cs.print('\n[bold on yellow]【Ok周志】: 这是最美好的一天啊，您也要开心哈:grinning_face_with_smiling_eyes:')
        cs.print(
        ...
        for i in range(timb, 0, -1):
            ...
            sleep(1)
        music.stop()
        cs.print('\n\n[bold on blue]【Ok周志]: 主人，音乐结束后我会自动关闭:zany_face:')
        sleep(1)
        music.load(r'C:\Ok周志\SysOk\B.mp3')
        music.play()
